...
def prepare(args, motd):
    ...
    url = "https://strawpoll.com/polls/wby5lv5ddZA" + args.id
    try:
        opt = int(args.option)
    except:
        print("Invalid option: '{3}'. Must be a number: 1 for 1st option, 2 for 2nd, ...".format(
            args.option))
        exit(1)
...
def vote(url, checkboxID, headers, proxy=None):
    ...
    try:
        ...
        page = requests.post(url, data={"security-token": secToken1, secToken2: "",
                             fieldName: checkboxID}, headers=headers, proxies=proxies, timeout=10).text
